...
void SpikingModel::run(float seconds, bool plasticity_on){
    ...
    cout<<"timestep_grouping: "<<timestep_grouping<<" number_of_timesteps: "<<number_of_timesteps<<"  number_of_steps: "<<number_of_steps<<endl;
    ...
    for (int s = 0; s < number_of_steps; s++){
        ...
        if(!(s%10000))
            cout<<"step: "<<s<<" of "<<number_of_steps<<" current_time_in_seconds: "<<current_time_in_seconds<<endl;
