        ...
        case CONNECTIVITY_TYPE_PAIRWISE:
            {
                ...
                for (int i=0; i < synapse_params->pairwise_connect_presynaptic.size(); i++){
                    ...
                    //.. prestart and poststart not needed since the pairwise_connect_presynaptic directly read the pre_ID from a file, same for poststart.
                    presynaptic_neuron_indices[original_number_of_synapses + i] = CORRECTED_PRESYNAPTIC_ID(int(synapse_params->pairwise_connect_presynaptic[i]), presynaptic_group_is_input);
                    postsynaptic_neuron_indices[original_number_of_synapses + i] = int(synapse_params->pairwise_connect_postsynaptic[i]);
                }
...
void Synapses::save_connectivity_as_binary(std::string path, std::string prefix, int synapsegroupid){
    ...
    //.. now dump the synapse startID and endID in a group ..
    std::ofstream synapse_start_end_ID_in_group_file;
    ...
    for(int i = 0; i<last_index_of_synapse_per_group.size(); i++) {
        ...
        synapse_start_end_ID_in_group_file<<startid<<" "<<endid<<std::endl;
    }
    synapse_start_end_ID_in_group_file.close();
}
